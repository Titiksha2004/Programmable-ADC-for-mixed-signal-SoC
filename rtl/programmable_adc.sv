...
// Technology:   40nm Ultra Low Power, Sky130 Open-Source PDK
// Author:       Vyges Development Team
// Created:     2025
//=============================================================================

module programmable_adc (
    // Clock and Reset
    ...
    programmable_adc_apb_interface apb_if (
        .PCLK(PCLK),
        .PRESETn(PRESETn),
        .PADDR(PADDR),
        .PWDATA(PWDATA),
    ...
    // PGA Stage
    programmable_adc_pga_stage pga_inst (
        .clk(PCLK),
        .reset_n(PRESETn),
        .enable(adc_enable),
        .gain(pga_gain),
    ...
    // Sample & Hold
    programmable_adc_sample_hold sh_inst (
        .clk(PCLK),
        .reset_n(PRESETn),
        .enable(adc_enable),
    ...
    // SAR Controller
    programmable_adc_sar_controller sar_ctrl (
        .clk(PCLK),
        .reset_n(PRESETn),
        .enable(adc_enable),
        .start_conv(start_conv),
    ...
    // Convert analog reference voltages to digital representation (for simulation)
    wire [15:0] vref_digital, vcm_digital;
    assign vref_digital = (VREF > 0) ? 16'hFFFF : 16'h0000;  // Full scale for VREF
    assign vcm_digital = (VCM > 0) ? 16'h8000 : 16'h0000;   // Mid-scale for VCM
    
    // DAC Array
    programmable_adc_dac_array dac_inst (
        .clk(PCLK),
        .reset_n(PRESETn),
        .enable(adc_enable),
        .resolution(resolution),
    ...
    // Comparator
    programmable_adc_comparator comp_inst (
        .clk(PCLK),
        .reset_n(PRESETn),
        .enable(adc_enable),
        .vin_p(sh_out_p),
